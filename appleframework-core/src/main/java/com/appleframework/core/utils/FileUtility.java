...
import static com.appleframework.core.utils.StringUtility.*;

import java.io.File;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

import com.appleframework.core.exception.IllegalPathException;
...
public class FileUtility {
    ...
    public static String getRelativePath(String basedir, String path) throws IllegalPathException {
        ...
        // 按"/"分隔basedir和path
        String[] baseParts = StringUtility.split(basedir, '/');
        String[] parts = StringUtility.split(path, '/');
    ...
    public static FileNameAndExtension getFileNameAndExtension(String path, boolean extensionToLowerCase) {
        path = StringUtility.trimToEmpty(path);
        ...
        if (!StringUtility.isEmpty(path)) {
            ...
            if (index >= 0) {
                extension = StringUtility.trimToNull(StringUtility.substring(path, index + 1));

                if (!StringUtility.containsNone(extension, "/\\")) {
                    extension = null;
                    index = -1;
                }
            }
            ...
            if (index >= 0) {
                fileName = StringUtility.substring(path, 0, index);
            }
        }
